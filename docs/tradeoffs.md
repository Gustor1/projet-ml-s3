...
- **Final Decision**: **Do NOT apply classical preprocessing globally in multi-task pipelines.** In systems requiring both ASR and SER (e.g., emotion-aware conversational interfaces), the raw audio must either be routed directly to the SER model (bypassing the noise filter), or the pipeline should utilize modern deep feature learning that handles noise natively. If classical preprocessing is active for the ASR model, implement a **parallel routing architecture**: route the denoised audio to the ASR model, and pass a peak-normalized, silence-trimmed version of the original audio to the SER model, calibrated in post-processing using our **multimodal fusion engine** (cross-referencing Whisper's text sentiment and the voice's pitch $F_0$).